...
Domain logic for article filtering - keyword-based blacklist filtering.
"""
...
def filter_by_blacklist(articles: List[Dict]) -> Tuple[List[Dict], List[Dict], Dict[str, int]]:
    ...
    for article in articles:
        text = f"{article['title']} {article.get('content', article.get('description', ''))}"
        is_blacklisted, matched_keyword = contains_blacklisted_keyword(text)

        if is_blacklisted:
            blacklisted_articles.append(article)
            blacklist_stats[matched_keyword] = blacklist_stats.get(matched_keyword, 0) + 1
        else:
...
def simple_filter(articles: List[Dict]) -> Tuple[List[Dict], Dict[str, Any]]:
    ...
    Fast and suitable for local execution.
